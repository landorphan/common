            ...
            else
            {
                    ...
                    parameters[1].Name.Equals("innerException", StringComparison.Ordinal))
                {
                    ...
                    try
                    {
                        ...
                        if (exception.InnerException != innerException)
                        {
                            rv.Add($"The exception type '{exceptionType.Name}' should not mangle the inner exception in the message, " +
                                   "innerException constructor, but it does mangle the inner exception.\n");
                        }
                    }
                    catch
                    {
                        rv.Add($"The exception type '{exceptionType.Name}' should not throw when its message, innerException constructor is invoked, but it does throw.\n");
                    }
                }
                else
                {
                    rv.Add($"The exception type '{exceptionType.Name}' should have a public constructor with the following signature: " +
